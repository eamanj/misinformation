...
summary(lm(correction_mimshare_numeric ~ tie_treatment + QMISINFO_BELIEF01, df))
summary(lm(correction_mimshare_numeric ~ tie_treatment + QMIM_DISCUSS_NEWS, df))
summary(lm(correction_mimshare_numeric ~ tie_treatment + QMIM_DISCUSS_NEWS + QMISINFO_BELIEF01, df))
...
summary(lm(correction_mimshare_numeric ~ tie_group_treatment + QMIM_DISCUSS_NEWS + resp_age + QEDUCATION_CHIEF, df))
...
summary(lm(correction_mimshare_numeric ~ tie_group_treatment + QTIE_POLI_FREQ + resp_age + QEDUCATION_CHIEF, df))
...
summary(lm(QCORR_MIMSHARE_BINARY ~ tie_treatment + QMIM_DISCUSS_NEWS + QMISINFO_BELIEF01, df))
...
summary(lm(QCORR_MIMSHARE_BINARY ~ tie_treatment + QMIM_DISCUSS_NEWS + resp_age + QEDUCATION_CHIEF, df))
...
summary(lm(QCORR_MIMSHARE_BINARY ~ tie_group_treatment + QMIM_DISCUSS_NEWS + resp_age + QEDUCATION_CHIEF, df))
        ...
        for(correct_correction in c(FALSE, TRUE)) {
          for(i in seq(1, length(control_vars_combs))) {
            for(quant in seq(0.1, 0.9, 0.05)) {
              ...
              formula <- paste0("correction_mimshare_numeric ~ inferred_tie_treatment + ", paste(controls, collapse=" + "),
              ...
              if(ivmodel_coef < 0 & ivmodel_pval < 0.01) {
