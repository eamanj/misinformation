...
PREVALENCE_BELIEF_PLATFORM_COLS <- paste0("claim_prevalence_belief_platform", seq(116))
...
for(i in seq(116)) {
  new_col <- PREVALENCE_BELIEF_PLATFORM_COLS[i]
  data[,new_col] <- paste0(data[[PREVALENCE_COLS[i]]], "_", data[[BELIEF_COLS[i]]],
                           "_", data[[PLATFORM_COLS[i]]])
}
...
generate_shown_claims_df <- function(df) {
  ...
  claims <- as.data.frame(apply(df[,msm_vars1] == "Yes", 1, function(x) names(x)[which(x == TRUE)]))
  ...
  claims$income_binary <- ifelse(df$income_numeric >= median(df$income_numeric, na.rm=T),
                                 "High", "Low")
  claims$education <- df$IN01EDU
  claims$education_numeric <- df$education_numeric
  claims$education_binary <- ifelse(df$education_numeric >= median(df$education_numeric, na.rm=T),
                                    "High", "Low")
...
gather_claims_by_columns <- function(df, claims, gather_cols, gather_col_name) {
  ...
  # convert claim code back to factor
  claims$claim_code <- factor(claims$claim_code, ordered = FALSE)
...
#claims$education <- df$IN01EDU
#claims$education_numeric <- df$education_numeric
#claims$education_binary <- ifelse(df$education_numeric >= median(df$education_numeric),
#                                  "High", "Low")
#claims$religion <- df$QRELIGION
#claims$modi_support <- df$modi_tweet
#claims$mim_discuss_news <- df$QMIM_DISCUSS_NEWS
...
#claims$politics_attention <- df$QPOLI_ATT
#claims$politics_interest <- df$QPOLI_INTEREST
#grouping_vars <- c('politics_interest')
...
ggplot(summary_stats, aes(x=prevalence, y=freq*100, fill=group)) +
  geom_bar(stat='identity', position='dodge') +
  ylab("Fraction within claim type") +
  xlab("Prevalence (Encountered before)") +
...
ggsave("./results/india/prevalence/prevalence_by_claim_type.png",  width = 10, height = 6)
...
ggsave("./results/india/prevalence/individual_prevalence.png",  width = 10, height = 6)
...
summary_stats
ggplot(summary_stats, aes(x=belief, y=freq*100, fill=group)) +
  geom_bar(stat='identity', position='dodge') +
  ylab("Fraction within claim type") +
  xlab("Belief (Now or when encountered)") +
  labs(fill="Claim Type") +
  ...
  theme(plot.title = element_text(hjust = 0.5, size=22),
        ...
        legend.title = element_text(size=20), 
        legend.text = element_text(size=18))
ggsave("./results/india/prevalence/belief_by_claim_type.png",  width = 10, height = 6)
...
ggplot(belief_df, aes(x=collapsed_belief, y=freq, fill=group, label=percent_freq)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size=27),
        ...
        legend.title = element_text(size=26), 
        legend.text = element_text(size=26))
ggsave("./results/india/prevalence/collapsed_belief_by_claim_type.png",  width = 10, height = 6)
...
ggplot(summary_stats, aes(x=belief, y=freq, group=group, fill=grouping)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size=22),
        ...
        legend.title = element_text(size=20), 
        legend.text = element_text(size=18))
ggsave("./results/india/prevalence/individual_belief.png",  width = 10, height = 6)
...
ggplot(summary_stats, aes(x=platform, y=freq, fill=group, label=percent_freq)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size=22),
        ...
        legend.title = element_text(size=20), 
        legend.text = element_text(size=18))
ggsave("./results/india/prevalence/platform_by_claim_type.png",  width = 10, height = 6)
...
ggplot(summary_stats, aes(x=platform, y=freq, group=group, fill=grouping)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size=22),
        ...
        legend.title = element_text(size=20), 
        legend.text = element_text(size=18))
ggsave("./results/india/prevalence/individual_platform.png",  width = 12, height = 6)



####################################################################################
# ANALYSIS OF PREVALENCE AND BELIEF AND PLATFORM
# evalute prevalence of misinfo claims versus mainstream and placebo claims
claims <- generate_shown_claims_df(df)
claims <- gather_claims_by_columns(df, claims, PREVALENCE_BELIEF_PLATFORM_COLS, "prevalence_belief_platform")
claims$prevalence <- gsub("_.*", "", claims$prevalence_belief_platform)
claims$belief <- gsub(".*_(.*)_.*", "\\1", claims$prevalence_belief_platform)
claims$platform <- gsub(".*_", "", claims$prevalence_belief_platform)
claims[claims$belief == "NA", "belief"] <- NA
claims[claims$platform == "NA", "platform"] <- NA

#claims <- claims[!claims$platform %in% c("Newspaper", "Television"),]
...
summary(lm(prevalence_numeric ~ claim_type + belief + platform, claims))
...
placebo_claims <- claims[claims$claim_type == "Placebo",]
grouping_vars <- c()
summary_stats <- placebo_claims %>%
  group_by_at(c(grouping_vars, "belief")) %>%
  mutate(group_count=n()) %>%
  group_by_at(c(grouping_vars, "belief", "prevalence")) %>%
  summarise(freq=n()/first(group_count))
if(is.null(grouping_vars)) {
  summary_stats$group <- summary_stats$belief
} else {
  summary_stats$group <- paste0(summary_stats$belief, '-', summary_stats[[grouping_vars]])
}

ggplot(summary_stats, aes(x=prevalence, y=freq, fill=group)) +
  geom_bar(stat='identity', position='dodge') +
  ylab("Fraction within Belief Group") +
  xlab("Have you encountered the claim Before?") +
  labs(fill="Perceived Accuracy") +
  ggtitle(paste('Probability of Prevalence Conditional on Perceived Accuracy (Placebo)')) +
  theme(plot.title = element_text(hjust = 0.5, size=20),
        axis.text.x = element_text(size=12),
        axis.text.y = element_text(size=12),
        axis.title.x = element_text(size=18),
        ...
        legend.title = element_text(size=18), 
        legend.text = element_text(size=16))
ggsave("./results/india/prevalence/placebo_prevalence_conditional_on_belief.png",  width = 12, height = 6)
...
grouping_vars <- c()
summary_stats <- placebo_claims %>%
  group_by_at(c(grouping_vars, "prevalence")) %>%
  mutate(group_count=n()) %>%
  group_by_at(c(grouping_vars, "prevalence", "belief")) %>%
  summarise(freq=n()/first(group_count))
if(is.null(grouping_vars)) {
  summary_stats$group <- summary_stats$prevalence
} else {
  summary_stats$group <- paste0(summary_stats$prevalence, '-', summary_stats[[grouping_vars]])
}

ggplot(summary_stats, aes(x=belief, y=freq, fill=group)) +
  geom_bar(stat='identity', position='dodge') +
  ylab("Fraction within Prevalence Group") +
  xlab("Perceived Accuracy of the Claim") +
  labs(fill="Prevalence") +
  coord_cartesian(ylim = c(0, 0.6))  +
  ggtitle(paste('Probability of Perceived Accuracy Conditional on Prevalence (Placebo)')) +
  theme(plot.title = element_text(hjust = 0.5, size=20),
        axis.text.x = element_text(size=12),
        axis.text.y = element_text(size=12),
        axis.title.x = element_text(size=18),
        ...
        legend.title = element_text(size=18), 
        legend.text = element_text(size=16))
ggsave("./results/india/prevalence/placebo_belief_conditional_on_prevalence.png",  width = 12, height = 6)